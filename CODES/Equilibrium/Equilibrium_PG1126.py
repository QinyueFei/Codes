...
# %%
path = "/media/qyfei/f6e0af82-2ae6-44a3-a033-f66b47f50cf4/ALMA/Type1AGN/PG_quasars/PG1126/data"
...
file_mom0 = "/PG1126-041_CO21_final_image_mom0.fits"
hdu = fits.open(path+file_mom0)[0]
mom0 = fits.open(path+file_mom0)[0].data[0][0]
file_mom2 = "/PG1126-041_CO21_final_image_mom2.fits"
mom2 = fits.open(path+file_mom2)[0].data[0][0]

# %%
rms = 0.056 #rms noise for intensity map
pix_size = abs(hdu.header['CDELT1']*u.deg.to('arcsec'))
radius = iso_rad(185, 185, np.array([88.5, 92.5]), pix_size, 145-90, 75)
...
coord = SkyCoord('10h14m20.69s -04d18m40.5s', frame='lsrk')
# pixels = w.world_to_pixel(coord, 300*u.GHz, 1)
coord = w[0, 0].pixel_to_world(89, 90)

# %%
pos_cen = np.array([88.5, 92.5])
...
ax.contour(mom2, levels=np.arange(10, 240, 20), linewidths=0.8, colors=['cyan'])
## overplot the intensity map contour and velocity dispersion map

ax.contour(radius, levels=np.array([2, 4]), linewidths=1, colors=['w'])
...
ax.scatter(92.5, 88.5, s=200, marker='x', color='b', label='KC')
...
# plt.savefig("/home/qyfei/Desktop/Results/Physical_values/Equilibrium/PG1126_show.pdf", bbox_inches="tight", dpi=300)
...
bea = np.where((radius[N][SN_threshold]<=2))
core = np.where((radius[N][SN_threshold]<=4) & (radius[N][SN_threshold]>2))
...
# plt.savefig("/home/qyfei/Desktop/Results/Physical_values/Equilibrium/PG1126_equilibrium.pdf", bbox_inches="tight", dpi=300)
