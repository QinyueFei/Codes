...
# %%
path = "/media/qyfei/f6e0af82-2ae6-44a3-a033-f66b47f50cf4/ALMA/Type1AGN/PG_quasars/PG1244/data"
...
file_mom0 = "/PG1244+026_CO21_final_image_mom0.fits"
hdu = fits.open(path+file_mom0)[0]
mom0 = fits.open(path+file_mom0)[0].data[0][0]
file_mom2 = "/PG1244+026_CO21_final_image_mom2.fits"
mom2 = fits.open(path+file_mom2)[0].data[0][0]

# %%
rms = 0.038 #rms noise for intensity map
pix_size = abs(hdu.header['CDELT1']*u.deg.to('arcsec'))
radius = iso_rad(170, 170, np.array([84.0, 83.5]), pix_size, 28-90, 32)
...
ax.contour(mom2, levels=np.arange(10, 240, 10), linewidths=0.8, colors=['cyan'])
## overplot the intensity map contour and velocity dispersion map

ax.contour(radius, levels=np.array([1, 3]), linewidths=1, colors=['w'])
...
ax.scatter(83.5, 84.0, s=200, marker='x', color='b', label='KC')
...
# rec = matplotlib.patches.Rectangle((pos_cen[0]-fov, pos_cen[1]+fov-rec_size), bar_size, rec_size, angle=0.0, fill=True, edgecolor='k', facecolor='w', zorder=2)
# ax.add_artist(rec)
# bar_length = abs(scale_bar_length.to('deg').value/delt)
ax.text(pos_cen[0]-fov+rec_size/8, pos_cen[1]+fov-rec_size/2, 'PG1244+026', color='w', fontsize=25, ha='left')
## represent the source name of the target

ax.set_xlim(pos_cen[0]-fov,pos_cen[0]+fov)
ax.set_ylim(pos_cen[1]-fov,pos_cen[1]+fov)

# plt.savefig("/home/qyfei/Desktop/Results/Physical_values/Equilibrium/PG1244_show.pdf", bbox_inches="tight", dpi=300)
...
bea = np.where((radius[N][SN_threshold]<=1))
core = np.where((radius[N][SN_threshold]<=3) & (radius[N][SN_threshold]>1))
...
# plt.savefig("/home/qyfei/Desktop/Results/Physical_values/Equilibrium/PG1244_equilibrium.pdf", bbox_inches="tight", dpi=300)
