...
f_err = 0.055#emom0*1e2                   ## The rms noise of the moment0 map
...
def log_likelihood(para, x, y, z, zerr):
    ...
    Ie2, Re2, n2, e2, t2 = para[20:]
    
    
    model_origin = Sersic_ring(hdu, x, y, x0, y0, Ie, Re, n, e, t, rbin, rsin, qin, PAin, rbout, rsout, qout, PAout) + Disk2D(hdu, x, y, x0, y0, Ie1, Re1, n1, e1, t1) + Disk2D(hdu, x, y, x0, y0, Ie2, Re2, n2, e2, t2)
...
def log_prior(para):
    ...
    if not (70<x0<100 and 70<y0<100 and 0.01<rbin and 0.0<rsin and 0<qin<=1 and -360<PAin<=360 and rbin<rbout and 0.0<rsout and 0<qout<=1 and -360<PAout<=360 and 1e-5<Ie<1e5 and 0.<Re<=20. and 0.01<n<10. and 0.01<e<=1 and -360<t<=360 and 1e-5<Ie1<1e5 and 0.<Re1<=20 and 0.01<n1<10. and 0.01<e1<=1 and -360<t1<=360 and 1e-5<Ie2<1e5 and 0.<Re2<=100 and 0.01<n2<10. and 0.01<e2<=1 and -360<t2<=360):
...
f_err = 0.055#emom0[mask]*1e3/10                   ## The rms noise of the moment0 map
# mom0_level = np.array([-1,1,2,4,8,16,32])*2*f_err
...
ini_guess = np.array([88.40, 89.66, 10.9, 1.1, 1.0, 0.1, 218,
                    ...
                    0.1, 0.4, 1.0, 0.2, 60, 0.2, 5.0, 1.0, 0.2, 60])


np.random.seed(42)
nll = lambda *args: -log_likelihood(*args)
initial = ini_guess + 0.1 * np.random.randn(25)
...
# %%
output_dir = "/home/qyfei/Desktop/Results/map_visualization/fitting/Results/PG0923/three_components/"
...
ini_guess = np.array([89.58, 88.10, 12.27, 1.70, 0.05, 0.40, 66.11, 
                    2.66, 0.86, 0.55, 68.79, 2.80, 0.01, 0.48, 78.80, 
                    0.54, 2.69, 0.47, 0.63, 39.46, 0.08, 5.85, 1.50, 0.02, 75.00])

pos = ini_guess + 1e-4 * np.random.randn(400, 25)
...
# %%
fig, axes = plt.subplots(25, figsize=(10, 25), sharex=True)
samples = sampler.get_chain()
labels = ["$x_0$", "$y_0$", "Ie", "Re", "n", "e", "t", "rin", "rsin", "qin", "PAin", "rout", "rsout", "qout", "PAout", "Ie1", "Re1", "n1", "e1", "t1", "Ie2", "Re2", "n2", "e2", "t2"]
for i in range(ndim):
    ax = axes[i]
...
output_dir = "/home/qyfei/Desktop/Results/map_visualization/fitting/Results/PG0923/EDGE/three_components/"
...
get_chain = np.reshape(chain[1800:], (400*200, 25))
...
f_disk = Disk2D(hdu, x, y, para_out[0], para_out[1], para_out[20], para_out[21], para_out[22], para_out[23], para_out[24])

f_model = f_ring + f_core + f_disk
...
# plt.savefig(output_dir+"PG0923_CO21_EDGE_three_components_fit.pdf", bbox_inches="tight", dpi=300)
...
ax1.text(10, 10, "BAR?", color="k")
im2 = ax2.imshow(f_disk, vmin=-vmin, vmax=vmax, cmap=cmap, origin='lower', norm=colors.PowerNorm(0.5))
ax2.contour(f_disk, mom0_level, colors=['k'], linewidths=1)
ax2.text(10, 10, "DISK", color="k")
...
# plt.savefig(output_dir+"PG0923_EDGE_three_components.pdf", bbox_inches="tight", dpi=300)
