...
f_err = emom0*1e2                   ## The rms noise of the moment0 map
...
def log_likelihood(para, x, y, z, zerr):
    ...
    Ie1, Re1, n1, e1, t1 = para[15:20]
    Ie2, Re2, n2, e2, t2 = para[20:25]
    Ie3, Re3, n3, e3, t3 = para[25:]
    
    
    
    model_origin = Sersic_ring(hdu, x, y, x0, y0, Ie, Re, n, e, t, rbin, rsin, qin, PAin, rbout, rsout, qout, PAout) + Disk2D(hdu, x, y, x0, y0, Ie1, Re1, n1, e1, t1) + Disk2D(hdu, x, y, x0, y0, Ie2, Re2, n2, e2, t2) + Disk2D(hdu, x, y, x0, y0, Ie3, Re3, n3, e3, t3)
...
def log_prior(para):
    ...
    if not (70<x0<100 and 70<y0<100 and 0.01<rbin and 0.0<rsin and 0<qin<=1 and -360<PAin<=360 and rbin<rbout and 0.0<rsout and 0<qout<=1 and -360<PAout<=360 and 1e-5<Ie<1e5 and 0.<Re<=20. and 0.01<n<10. and 0.01<e<=1 and -360<t<=360 and 1e-5<Ie1<1e5 and 0.<Re1<=20 and 0.01<n1<10. and 0.01<e1<=1 and -360<t1<=360 and 1e-5<Ie2<1e5 and 0.<Re2 and 0.01<n2<10. and 0.01<e2<=1 and -360<t2<=360 and 1e-5<Ie3<1e5 and 0.<Re3 and 0.01<n3<10. and 0.01<e3<=1 and -360<t3<=360):
...
f_err = emom0[mask]*1e3/10                   ## The rms noise of the moment0 map
# mom0_level = np.array([-1,1,2,4,8,16,32])*2*f_err
...
ini_guess = np.array([88.40, 89.66, 10.9, 1.1, 1.0, 0.1, 218,
...
initial = ini_guess + 0.1 * np.random.randn(30)
...
print("Ie2 = {0:.3f}".format(para_mini[20]))
print("Re2 = {0:.3f}".format(para_mini[21]))
print("n2 = {0:.3f}".format(para_mini[22]))
print("e2 = {0:.3f}".format(para_mini[23]))
print("t2 = {0:.3f}".format(para_mini[24]))

print("Ie3 = {0:.3f}".format(para_mini[25]))
print("Re3 = {0:.3f}".format(para_mini[26]))
print("n3 = {0:.3f}".format(para_mini[27]))
print("e3 = {0:.3f}".format(para_mini[28]))
print("t3 = {0:.3f}".format(para_mini[29]))
# %%
output_dir = "/home/qyfei/Desktop/Results/map_visualization/fitting/Results/PG0923/EDGE/four_components/"
...
ini_guess = np.array([89.59, 88.12, 17.24, 1.56, 0.15, 0.37, 60.34, 
                    2.77, 1.14, 0.57, 67.19, 2.81, 0.01, 0.48, 78.92, 
                    0.52, 2.77, 0.25, 0.59, 37.17, 
                    0.1, 0.1, 1.0, 0.11, 83.66,
                    0.1, 3.0, 1.0, 0.05, 83])

pos = ini_guess + 1e-4 * np.random.randn(400, 30)
...
# %%
fig, axes = plt.subplots(30, figsize=(10, 30), sharex=True)
samples = sampler.get_chain()
labels = ["$x_0$", "$y_0$", "Ie", "Re", "n", "e", "t", "rin", "rsin", "qin", "PAin", "rout", "rsout", "qout", "PAout", "Ie1", "Re1", "n1", "e1", "t1", "Ie2", "Re2", "n2", "e2", "t2", "Ie3", "Re3", "n3", "e3", "t3"]
for i in range(ndim):
    ax = axes[i]
...
get_chain = np.reshape(chain[1800:], (400*200, 30))
...
fig = corner.corner(
        get_chain, labels=labels, truths=[para_out[0],para_out[1],para_out[2],para_out[3],para_out[4],para_out[5],para_out[6],para_out[7],para_out[8],para_out[9],para_out[10],para_out[11], para_out[12], para_out[13], para_out[14], para_out[15], para_out[16], para_out[17], para_out[18], para_out[19], para_out[20], para_out[21], para_out[22], para_out[23], para_out[24], para_out[25], para_out[26], para_out[27], para_out[28], para_out[29]], show_titles=True
    )#
...
# para_out = np.array([88.40, 89.60, 33.84, 0.93, 1.80, 0.26, -41.05, 
#                     1.88, 0.01, 0.62, 72.98, 3.53, 1.58, 0.42, 70.99, 
#                     0.38, 3.05, 0.04, 0.59, 37.98, 0.01, 19.87, 3.55, 0.07, 87.31])
...
f_core = Disk2D(hdu, x, y, para_out[0], para_out[1], para_out[15], para_out[16], para_out[17], para_out[18], para_out[19])
f_disk1 = Disk2D(hdu, x, y, para_out[0], para_out[1], para_out[20], para_out[21], para_out[22], para_out[23], para_out[24])
f_disk2 = Disk2D(hdu, x, y, para_out[0], para_out[1], para_out[25], para_out[26], para_out[27], para_out[28], para_out[29])


f_model = f_ring + f_core + f_disk1 + f_disk2
...
ax0.add_artist(rec)
Beam = beam(hdu, 5., 5., 'gray', pix_size)
ax0.add_artist(Beam[0])

# plt.savefig(output_dir+"PG0923_CO21_EDGE_four_components_fit.pdf", bbox_inches="tight", dpi=300)
...
im1 = ax1.imshow(f_core, vmin=-vmin, vmax=vmax, cmap=cmap, origin='lower', norm=colors.PowerNorm(0.5))
ax1.contour(f_core, mom0_level, colors=['k'], linewidths=1)
ax1.text(10, 10, "CORE", color="k")
im2 = ax2.imshow(f_disk1, vmin=-vmin, vmax=vmax, cmap=cmap, origin='lower', norm=colors.PowerNorm(0.5))
ax2.contour(f_disk1, mom0_level, colors=['k'], linewidths=1)
ax2.text(10, 10, "DISK1", color="k")
im3 = ax3.imshow(f_disk2, vmin=-vmin, vmax=vmax, cmap=cmap, origin='lower', norm=colors.PowerNorm(0.5))
ax3.contour(f_disk2, mom0_level, colors=['k'], linewidths=1)
ax3.text(10, 10, "DISK2", color="k")

for ax in axes[:]:
    ax.xaxis.set_ticklabels([])
    ax.yaxis.set_ticklabels([])
    ax.scatter(88.13, 89.58, marker='+', color='k', s=100)
...
# plt.savefig(output_dir+"PG0923_EDGE_four_components.pdf", bbox_inches="tight", dpi=300)
