# %%
## Fit with truncated Sersic model
...
def log_likelihood(para, x, y, z, zerr):
    ...
    model = truncated_Disk2D_ring(hdu, x, y, x0, y0, Ie, Re, n, e, t, r_break, dr_soft, q, theta_PA)
    sigma2 = zerr ** 2
...
def log_prior(para):
    ...
    if not (-10<x0<10 and -10<y0<10 and 0.01<r_break and 0.01<dr_soft and 0<q<=1 and -360<theta_PA<=360 and 1e-5<Ie<1e5 and 0.<Re<=20. and 0.01<n<10. and 0.01<e<=1 and -360<t<=360):
...
# %%
output_dir = "/home/qyfei/Desktop/Results/map_visualization/fitting/Results/PG0923/truncated_sersic/"
print("Begin mcmc fitting:")
from multiprocessing import Pool
import emcee

fit_results = para_ini
...
# %%
# %%
fig, axes = plt.subplots(11, figsize=(10, 11), sharex=True)
samples = sampler.get_chain()
labels = ["$x_0$", "$y_0$", "Ie", "Re", "n", "e", "t", "$r_b$", "$dr_s$", "q", "PA"]
for i in range(ndim):
    ax = axes[i]
...
get_chain = np.reshape(chain[1800:], (400*200, 11))

# %%
labels = ["$x_0$", "$y_0$", "Ie", "Re", "n", "e", "t", "$r_b$", "$dr_s$", "q", "PA"]#, "Ie2", "Re2", "n2", "e2", "t2"
...
f_model = truncated_Disk2D_ring(hdu, x, y, para_out[0],para_out[1],para_out[2],para_out[3],para_out[4],para_out[5],para_out[6],para_out[7],para_out[8],para_out[9],para_out[10])
