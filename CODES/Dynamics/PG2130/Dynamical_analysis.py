...
path = "/media/qyfei/f6e0af82-2ae6-44a3-a033-f66b47f50cf4/ALMA/Type1AGN/PG_quasars/PG2130/Barolo_fit/output/PG2130+099"
file = "/ringlog2.txt"
...
convert = Planck15.arcsec_per_kpc_proper(0.061).value
log_Mb = 10.43
re = 1.00 / convert
n = 4.0
...
def log_likelihood(theta, x, y, yerr, vasy):
    log_Mb, re, n, log_Mtot, rd, log_Mdh, con, alpha_CO = theta #Mb, re, n, Md, rd, 
...
def log_prior(theta):
    ...
    ## constraints
    ## Prior knowledge
    ## logMb = 10.43\pm0.47, logMtot = 10.74\pm0.50
    ## reb = 

    if not (8<log_Mb<15 and 8<log_Mtot<15 and log_Mb<log_Mtot and -2.25<log_fb<-1.3 and (log_con_theory-0.1)<log_con<(log_con_theory+0.1) and 0<alpha_CO<20 and 0.1<re<10.0 and 0.6<n<10.0 and 3.<rd<15.): 
    # if not (9.96<log_Mb<10.96 and 10.24<log_Mtot<11.24 and log_Mb<log_Mtot and -2.25<log_fb<-1.3 and (log_con_theory-0.1)<log_con<(log_con_theory+0.1) and 0<alpha_CO<20):
        return -np.inf
    Mbmu = 10.43
    Mbsigma = 0.47
    ...
    lp = lpre + lpn + lpred + lp_alpha_CO#+ lpMb + lpMt 
    #return 0
    # lp = 0
    return lp
...
initial = np.array([log_Mb, re, n, log_Mtot, rd, log_Mdh, con, a]) + 0.1 * np.random.randn(8)#Mb, re, n, Md, rd, 
...
print("log_Mtot = {0:.3f}".format(log_Md_ml))
...
# 10.40, 1.0, 2.5, 10.80, 5.0, 12.20, 6.00, 4.34
...
log_Mb, re, n, log_Mtot, rd, log_Mdh, log_fb, con, a = 10.33, 2.91, 3.47, 10.77, 5.44, 12.99, -2.22, 7.92, 7.30
...
pos = [log_Mb, re, n, log_Mtot, rd, log_Mdh, con, a] + 1e-4 * np.random.randn(400, 8)

# Mb, re, n, Md, rd, 
#[Mb_ml, re_ml, n_ml, Md_ml, rd_ml, Mdh_ml] + 1e-4 * np.random.randn(32, 6)
nwalkers, ndim = pos.shape
output_dir = "/home/qyfei/Desktop/Results/Dynamics/results/PG2130/rotation_velocity/CO_based/alpha_CO_constrained_one_sigma_gauss/"
...
get_chain = np.reshape(chain[400:], (400*600, 8))
...
# %%
labels = ["$\log M_\mathrm{b}$", "$r_\mathrm{e,b}$", "$n$", "$\log M_\mathrm{tot}$", "$r_\mathrm{e,d}$", "$\log M_\mathrm{DM}$","c","$\\alpha_\mathrm{CO}$"]
...
r_fit_tot = r_fit#[2:10]
vrot_tot = vrot_fit#[2:10]
evrot_tot = evrot2_fit#[2:10]
vasy_tot = vasy#[2:10]

parameters = np.array(para_out)[:,1]
# parameters = soln.x
# parameters = soln.x#np.array([10.40, 1.0, 2.5, 10.80, 5.0, 12.20, 6.00, 4.34])
...
ax.plot(r_test, np.sqrt(v_b**2+v_d**2+v_dh**2+V_gas**2*alpha_CO - vasy**2), "Grey", lw=6, zorder=0, label='Total')

ax.errorbar(r_fit, vrot_fit, yerr=[-evrot1_fit, evrot2_fit], fmt='bo', mfc='b', ms=10, mew=1, elinewidth=1, capsize=5)
ax.errorbar(r_fit[2:10], vrot_fit[2:10], yerr=[-evrot1_fit[2:10], evrot2_fit[2:10]], fmt='ro', mfc='r', ms=10, mew=1, elinewidth=1, capsize=5, zorder=5)

ax.fill_between(r_fit, vrot_fit+evrot1_fit, vrot_fit+evrot2_fit, color='b', alpha=0.3, zorder=3)
...
# %%
output_dir = "/home/qyfei/Desktop/Results/Dynamics/results/PG0923/rotation_velocity/CO_based"

documents = ["/stellar_mass_constrained_one_sigma_boundary/", "/stellar_mass_constrained_three_sigma_boundary/", "/alpha_CO_constrained_prior/", "/no_constraints/"]
...
ax0.plot(r_test, V_gas*np.sqrt(alpha_CO), "C4", lw=2, label="Gas")

ax0.plot(r_test, np.sqrt(v_b**2+v_d**2+v_dh**2+V_gas**2*alpha_CO - vasy_tot**2), "Grey", lw=6, zorder=0, label='Total')

ax0.errorbar(r_fit, vrot_fit, yerr=[-evrot1_fit, evrot2_fit], fmt='bo', mfc='b', ms=10, mew=1, elinewidth=1, capsize=5)
ax0.fill_between(r_fit, vrot_fit+evrot1_fit, vrot_fit+evrot2_fit, color='b', alpha=0.3, zorder=3)
...
ax0.set_xlim(1e-4, r_fit[-1]+(r_fit[2]-r_fit[1])/2)
ax0.set_ylim(-5, 335)
ax0.grid()
ax0.fill_between([0, 2*(r_fit[1]-r_fit[0])], [-500], [500], color='k', hatch='/', alpha=0.1)
...
f = h5py.File(output_dir+"/stellar_mass_fixed/"+"tutorial.h5", "r")
accepted = f['mcmc']['accepted']
chain = f['mcmc']['chain']
log_prob = f['mcmc']['log_prob']
get_chain = np.reshape(chain[600:], (400*400, 6))
alpha_CO_distribution = get_chain[:,5]
alpha_CO_mean = np.percentile(alpha_CO_distribution, 50)
numbers = np.zeros(len(bins))
for j in range(len(bins)):
    N = np.where((alpha_CO_distribution>=bins[j]-dbins) & (alpha_CO_distribution<bins[j]+dbins))
        #print(N)
    numbers[j] = len(N[0])
ax1.plot(bins, numbers/np.nanmax(numbers), drawstyle = "steps", ls="-", lw=3, color='k', label='A.3')
...
plt.savefig("/home/qyfei/Desktop/Results/Dynamics/results/PG0923/rotation_velocity/CO_based/RC_fit_alpha_CO_distribution_extra.pdf", bbox_inches="tight", dpi=300)
