# %%
## This file is as same as original code, while in this case some parameters are fixed.
...
def log_likelihood(theta, x, y, yerr, z, zerr):
    ## fix the bulge and disk mass
    re, n, rd, log_Mdh, con, alpha_CO = theta #Mb, re, n, Md, rd, 
    log_Mb, log_Md = 10.96, 10.64
...
def log_prior(theta):
    ...
    if not (0-2.25<log_fb<-1.3 and (log_con_theory-0.1)<log_con<(log_con_theory+0.1) and 0.0<alpha_CO<20): #
        return -np.inf
    Mbmu = 10.96
    Mbsigma = 0.50
...
initial = np.array([re, n, rd, log_Mdh, con, a]) + 0.1 * np.random.randn(8)#Mb, re, n, Md, rd, 
...
pos = [re, n, rd, log_Mdh, con, a] + 1e-4 * np.random.randn(400, 6)

# Mb, re, n, Md, rd, 
#[Mb_ml, re_ml, n_ml, Md_ml, rd_ml, Mdh_ml] + 1e-4 * np.random.randn(32, 6)
nwalkers, ndim = pos.shape
output_dir = "/home/qyfei/Desktop/Results/Dynamics/results/PG0050/rotation_velocity/CO_based/new_without_spiral/stellar_mass_fixed/"
...
fig, axes = plt.subplots(6, figsize=(10, 40), sharex=True)
samples = sampler.get_chain()
labels = ["re", "n", "rd", "logMdh", "con", "alpha"] #Mb, re, n, Md, rd, 
for i in range(ndim):
    ax = axes[i]
...
import matplotlib.pyplot as plt
import corner
#output_dir = "/home/qyfei/Desktop/Results/Dynamics/results/PG0050/rotation_velocity/CO_based/new_without_spiral/stellar_mass_constrained_one_sigma/"
labels = ["r_{e,b}", "n", "r_{e,d}", "log M_{DM}","C","\\alpha_{CO}"]
...
# %%
labels = ["$r_\mathrm{e,b}$", "$n$", "$r_\mathrm{e,d}$", "$\log M_\mathrm{DM}$","c","$\\alpha_\mathrm{CO}$"]
...
fig = corner.corner(get_chain[:, [0,1,2,3,4,5]], labels=labels, show_titles=True)
...
#r_fit_tot = r_fit#[7:]
#vrot_tot = vrot_fit#[7:]
#evrot_tot = evrot2_fit#[7:]
#disp_tot = disp_fit#[7:]
#edisp_tot = edisp2_fit#[7:]

parameters = np.array(para_out)[:,1]

alpha_CO = parameters[5]
...
ax = plt.subplot(111)
v_b = V_b(r_test, 10.96, parameters[0], parameters[1]) #parameters[0], parameters[1], parameters[2]
v_d = V_d(r_test, 10.64, parameters[2]) #parameters[3], parameters[4]
#log_Mdh = np.log10((10**parameters[0]+10**parameters[3]+parameters[7]*1.36*L_CO)*(1/10**parameters[5]-1))
v_dh = V_dh(r_test, parameters[3], parameters[4])
...
ax.plot(r_test, np.sqrt(v_b**2+v_d**2+v_dh**2+v_g**2*alpha_CO + disp_tot**2*(asy0+asy1)), "Grey", lw=6, zorder=3, label='Total')
...
#plt.savefig(output_dir+'RC_fit.pdf', bbox_inches='tight', dpi=300)
