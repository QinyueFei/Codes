...
def log_prior(theta):
    ...
    #if not (9.46<log_Mb<12.46 and 0.1<re<10.0 and 0.6<n<10.0 and 9.14<log_Md<12.14 and 5.<rd<15. and -2.25<log_fb<-1.3 and (log_con_theory-0.1)<log_con<(log_con_theory+0.1) and 0.0<alpha_CO<20): #
    ## no constraints
    if not (8.0<log_Mb<15.0 and 8.0<log_Md<15.0 and -2.25<log_fb<-1.3 and (log_con_theory-0.1)<log_con<(log_con_theory+0.1) and 0.1<alpha_CO<20.0): 
        return -np.inf
    Mbmu = 10.96
    Mbsigma = 0.50
...
soln = minimize(nll, initial, args=(r_fit_tot, vrot_tot, evrot_tot), method="Nelder-Mead")#, disp_tot, edisp_tot
...
output_dir = "/home/qyfei/Desktop/Results/Dynamics/results/PG0050/rotation_velocity/CO_based/no_adc/no_constraints_test/"
...
with Pool() as pool:
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability, args=(r_fit_tot, vrot_tot, evrot_tot), backend=backend) #, disp_tot, edisp_tot
...
ax.plot(r_test, np.sqrt(v_b**2+v_d**2+v_dh**2+v_g**2*alpha_CO), "Grey", lw=6, zorder=3, label='Total')# + disp_tot**2*(asy0+asy1)
