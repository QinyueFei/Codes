...
VROT_range = np.arange(ext[0], ext[1]+1.5, 1.5)
VDISP_range = np.arange(ext[2], ext[3]+1, 1)

contour_level = np.percentile(d, [30, 50, 84])

for i in range(nrad):
    f_res = d[i]

    # contour_level = np.percentile(f_res, [16, 50, 84])
...
# np.savetxt("/media/qyfei/f6e0af82-2ae6-44a3-a033-f66b47f50cf4/ALMA/Type1AGN/PG_quasars/PG0923/Barolo_fit/output/PG0923+129/vel_prof.txt", np.array([mean_VROT, mean_VDISP, sigma_VROT, sigma_VDISP]).T)
