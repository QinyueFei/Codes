...
ax = plt.subplot(111)
ax.imshow(f_mom0, origin='lower', vmin=f_err, vmax=np.percentile(f_mom0, [99.9]), cmap='jet', norm=LogNorm())
...
def log_prior(para):
    ...
    if not (-1<x0<1 and -1<y0<1 and 1e-5<Ie<20. and 0.<Re<5. and 0.0<n<10.0 and 0<e<=1 and 0<t<=360):
        return -np.inf
...
#def fit_mcmc(f_mom0):
fit_results = [0.037, 0.048, 5.967, 0.237, 4.192, 0.593, 175]#fit_mini(f_mom0)
...
labels = ["$x_0$", "$y_0$", "Ie", "Re", "n", "e", "t", "$log_f$"]
for i in range(ndim):
    ax = axes[i]
...
im0 = ax0.imshow(f_mom0, vmin=vmin, vmax=vmax, cmap='jet', origin='lower')
...
im2 = ax2.imshow(f_total_res, vmin=vmin, vmax=-vmin, cmap='jet', origin='lower')
...
rec = matplotlib.patches.Rectangle((0, 0), 14, 14,
angle=0.0,fill=True, edgecolor='k', facecolor='w', zorder=2)
ax0.add_artist(rec)
Beam = beam(hdu, 7., 7., 'w', pix_size)
