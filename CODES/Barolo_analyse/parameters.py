...
ax2, ax0 = axes
ax0.errorbar(r_fit, pa_fit, fmt='mo', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0.)
ax0.set_ylabel('$\phi$ [deg]')
ax0.set_xlim(-0.1, 3.2)
ax0.set_ylim(80, 150)
ax0.set_xlabel('radius [kpc]')
ax1 = ax0.twinx()
ax1.errorbar(r_fit, inc_fit, fmt='gs', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0)
ax1.set_ylabel('$i$ [deg]')
ax1.set_ylim(30, 80)

ax1.errorbar(0,0,0, fmt='mo', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0, label='$\phi$')
ax1.errorbar(0,0,0, fmt='gs', mfc='none',ms=10, mew=0.5, elinewidth=0.5, capsize=0, label='$i$')
ax1.legend(loc='upper right')

## plot velocity profile
ax2.errorbar(r_fit, vcirc_fit, yerr=[-evrot1_fit, evrot2_fit], fmt='bo', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0)
ax2.errorbar(r_fit, vrot_fit, yerr=[-evrot1_fit, evrot2_fit], fmt='co', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0)
...
ax3 = ax2.twinx()
ax3.errorbar(r_fit, vdisp_fit, fmt='rs', mfc='none', ms=10, mew=0.5, elinewidth=0.5, capsize=0)
ax3.set_ylabel('$\sigma$ [$\mathrm{km\,s^{-1}}$]')
ax3.set_ylim(5, 110)

ax3.errorbar(0,0,0, fmt='co', mfc='none', ms=10, mew=0.5, capsize=0, elinewidth=0.5, label='$v_\mathrm{rot}$')
ax3.errorbar(0,0,0, fmt='bo', mfc='none', ms=10, mew=0.5, capsize=0, elinewidth=0.5, label='$v_\mathrm{circ}$')
ax3.errorbar(0,0,0, fmt='rs', mfc='none',ms=10, mew=0.5, capsize=0, elinewidth=0.5, label='$\sigma$')
ax3.legend(loc='upper right')
for ax in axes:
    ax.fill_between([0,0.44], [0], [500], color='b', alpha=0.2)
    ax.vlines(0.8, 0, 500, color='b', lw=0.5, ls='--')
    ax.vlines(2.1, 0, 500, color='b', lw=0.5, ls='--')
plt.show()
'''
#plt.savefig('/home/qyfei/Desktop/Codes/Result/PG0050/Barolo/profiles.pdf', bbox_inches='tight', dpi=300)
